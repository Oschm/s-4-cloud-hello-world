...
public class GetSingleBusinessPartnerByIdCommand {
    ...
    public BusinessPartner execute() throws Exception {
        ...
        return service.getAllBusinessPartner().filter(BusinessPartner.BUSINESS_PARTNER.eq(id))
                .select(BusinessPartner.ALL_FIELDS, BusinessPartner.TO_BUSINESS_PARTNER_ADDRESS).execute().get(0);
